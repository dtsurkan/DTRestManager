...
unichar VBISO8601DefaultTimeSeparatorCharacter = ':';
...
@interface VBISO8601DateFormatter(UnparsingPrivate)
...
@interface VBISO8601DateFormatter ()
...
@implementation VBISO8601DateFormatter
...
- (id)init
{
	...
	if (self) {
		...
		self.format = VBISO8601DateFormatCalendar;
		self.timeSeparator = VBISO8601DefaultTimeSeparatorCharacter;
		self.includeTime = NO;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange\
{
	...
	if (strict) timeSep = VBISO8601DefaultTimeSeparatorCharacter;
...
{
	switch (self.format) {
		case VBISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:RK_ISOCALENDAR_DATE_FORMAT];
		case VBISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:self.timeZone];
		case VBISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:RK_ISOORDINAL_DATE_FORMAT];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %ld, not calendar (%d), week (%d), or ordinal (%d)", (unsigned long) self.format, VBISO8601DateFormatCalendar, VBISO8601DateFormatWeek, VBISO8601DateFormatOrdinal];
			return nil;
	}
}
...
- (NSString *)weekDateStringForDate:(NSDate *)date
{
	...
	if (self.includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = VBISO8601DefaultTimeSeparatorCharacter;
